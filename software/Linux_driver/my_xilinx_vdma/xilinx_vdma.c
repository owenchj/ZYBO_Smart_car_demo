...
#define MY_HSIZE		        0x00000780
#define MY_VSIZE	                0x000001E0 
#define MY_STRIDE	                0x00000780

#define MY_HSIZE_FILTER		        0x00000280
#define MY_VSIZE_FILTER	                0x000001E0 
#define MY_STRIDE_FILTER                0x00000280

#define VDMA_0_ADDR                     0x43000000
#define VDMA_1_ADDR                     0x43010000

int VDMA0_ADDR[3] = {0x1C000000, 0x1C0E1000, 0x1C1C2000};
int VDMA1_ADDR[3] = {0x1C600000, 0x1C64B000, 0x1C696000};


volatile int result = 0; 
volatile int result_filter = 0;
volatile int index[3] = {0,0,0};
volatile int index_filter[3] = {0,0,0};
...
volatile int already_write_index = 0 ; 
char WI[6] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

volatile int irq_num_filter = 0 ; 
volatile int read_index_filter = 0 ; 
char RI_filter = {0x00};

volatile int write_index_filter = 0 ; 
volatile int already_write_index_filter = 0 ; 
char WI_filter[6] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
static void xilinx_vdma_start_transfer(struct xilinx_vdma_chan *chan)
{
  ...
  if (chan->has_sg) {
    ...
    dev_info(chan->dev, "hsize = %d || stride = %d || vsize = %d || \n", last->hw.hsize, last->hw.stride, last->hw.vsize); 
...
static irqreturn_t xilinx_vdma_irq_handler(int irq, void *data)
{
  ...
  if (status & XILINX_VDMA_DMASR_FRM_CNT_IRQ) {
    ...
    //status = of_address_to_resource((chan->dev->of_node), 0, res);
    //dev_info(chan->dev, "irq_%x\n", res->start);  

    
    status = (chan->xdev)->regs;
    ...
    // dev_info(chan->dev, "add = %x; vdma0 = %x; %d\n", status, vdma_0, flag); 

    if( status == vdma_0 ){
           
      already_write_index = write_index;      
      ...
      //      if(irq_num < 50){
        /* dev_info(chan->dev, "read_index = %d || write_index = %d || al_write_index = %d %d || \n", read_index, write_index, already_write_index, irq_num); */
        /* dev_info(chan->dev, "0 = %d || 1 = %d || 2 = %d || \n", index[0], index[1], index[2]); */
	//}
      
      vdma_desc_write(chan, XILINX_VDMA_REG_START_ADDRESS(0), VDMA0_ADDR[index[0]]);
      vdma_desc_write(chan, XILINX_VDMA_REG_START_ADDRESS(1), VDMA0_ADDR[index[1]]);
      vdma_desc_write(chan, XILINX_VDMA_REG_START_ADDRESS(2), VDMA0_ADDR[index[2]]);
      vdma_desc_write(chan, XILINX_VDMA_REG_HSIZE, MY_HSIZE);
      vdma_desc_write(chan, XILINX_VDMA_REG_FRMDLY_STRIDE, MY_STRIDE);
      vdma_desc_write(chan, XILINX_VDMA_REG_VSIZE, MY_VSIZE);
    }
    else{
      
      already_write_index_filter = write_index_filter;      
      irq_num_filter++; 
      
      result_filter = irq_num_filter % 3; 
      
      write_index_filter = index_filter[result_filter];

      if(read_index_filter == 0){
    	index_filter[0] = 1;
    	index_filter[1] = 1;
    	index_filter[2] = 2;
      }
      else if(read_index_filter == 1){
    	index_filter[0] = 0;
    	index_filter[1] = 2;
    	index_filter[2] = 2;
      }
      ...
      /* if(irq_num_filter < 50){ */
      /*   dev_info(chan->dev, "read_index = %d || write_index = %d || al_write_index = %d || \n", read_index_filter, write_index_filter, already_write_index_filter); */
      /*   dev_info(chan->dev, "f0 = %d || f1 = %d || f2 = %d || \n", index_filter[0], index_filter[1], index_filter[2]); */
      /* } */

      vdma_desc_write(chan, XILINX_VDMA_REG_START_ADDRESS(0), VDMA1_ADDR[index_filter[0]]);
      vdma_desc_write(chan, XILINX_VDMA_REG_START_ADDRESS(1), VDMA1_ADDR[index_filter[1]]);
      vdma_desc_write(chan, XILINX_VDMA_REG_START_ADDRESS(2), VDMA1_ADDR[index_filter[2]]);
      vdma_desc_write(chan, XILINX_VDMA_REG_HSIZE, MY_HSIZE_FILTER);
      vdma_desc_write(chan, XILINX_VDMA_REG_FRMDLY_STRIDE, MY_STRIDE_FILTER);
      vdma_desc_write(chan, XILINX_VDMA_REG_VSIZE, MY_VSIZE_FILTER);
    }
...
static int xilinx_vdma_open_filter(struct inode *inode, struct file *file)
{
  struct miscdevice *mdev = file->private_data;
  struct xilinx_vdma_chan *chan;

  chan = container_of(mdev, struct xilinx_vdma_chan, miscdev);

  return 0;
}

long xilinx_vdma_unlocked_ioctl_filter(struct file *file, unsigned int cmd, unsigned long arg){

  struct miscdevice *mdev = file->private_data;
  struct xilinx_vdma_chan *s;

  s = container_of(mdev, struct xilinx_vdma_chan, miscdev);
  ...
  case XVDMA_START:
    {
      ...
      // xilinx_vdma_start((struct xilinx_vdma_chan *)chan);
      
      break;
    }
  case XVDMA_GET_IRQ_NUM:
    
    break;

  case XVDMA_RESET_IRQ_NUM:
    {
      irq_num_filter = 0;
      write_index_filter = 0;
      already_write_index_filter = 0;
      read_index_filter = 0;
      break;
    }
  default:
...
//!!
static ssize_t xilinx_vdma_read_filter(struct file *file, char __user *buf, size_t count, loff_t *f_pos){
  struct miscdevice *mdev = file->private_data;
  struct xilinx_vdma_chan *chan;
  int err;
  ...
  WI_filter[0] = (irq_num_filter >> 24) & 0xFF;
  WI_filter[1] = (irq_num_filter >> 16) & 0xFF;
  WI_filter[2] = (irq_num_filter >> 8) & 0xFF;
  WI_filter[3] = irq_num_filter & 0xFF;
  WI_filter[4] = write_index_filter & 0xFF;  
  WI_filter[5] = already_write_index_filter & 0xFF;  
  err = copy_to_user(buf, WI_filter, count);

  
  if (err < 0){
    dev_err(chan->dev,"failed to copy to user space, total = %d\n", count);
    return err;
  }
  return count;
}

static ssize_t xilinx_vdma_write_filter(struct file *file, const char __user *buf, size_t count, loff_t *f_pos){
  struct miscdevice *mdev = file->private_data;
  struct xilinx_vdma_chan *chan;
  int err;
  chan = container_of(mdev, struct xilinx_vdma_chan, miscdev);

  err = copy_from_user(&RI_filter, buf, count);
  read_index_filter = 0; 
  read_index_filter |= RI_filter;
  dev_info(chan->dev, "Read Idex_filter = %d\n", read_index_filter); 
...
static int xilinx_vdma_open(struct inode *inode, struct file *file)
{
  struct miscdevice *mdev = file->private_data;
  struct xilinx_vdma_chan *chan;
...
long xilinx_vdma_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg){
...
//!!
static ssize_t xilinx_vdma_read(struct file *file, char __user *buf, size_t count, loff_t *f_pos){
  struct miscdevice *mdev = file->private_data;
  struct xilinx_vdma_chan *chan;
  int err;
  ...
  WI[0] = (irq_num >> 24) & 0xFF;
  WI[1] = (irq_num >> 16) & 0xFF;
  WI[2] = (irq_num >> 8) & 0xFF;
  WI[3] = irq_num & 0xFF;
  WI[4] = write_index & 0xFF;  
  WI[5] = already_write_index & 0xFF;  
...
static ssize_t xilinx_vdma_write(struct file *file, const char __user *buf, size_t count, loff_t *f_pos){
  struct miscdevice *mdev = file->private_data;
  struct xilinx_vdma_chan *chan;
  int err;
  chan = container_of(mdev, struct xilinx_vdma_chan, miscdev);
...
// fops
static const struct file_operations xilinx_vdma_original_fops = {
  .owner = THIS_MODULE,
  //	.llseek = no_llseek,
  .read  = xilinx_vdma_read,
  .write = xilinx_vdma_write,
  .open  = xilinx_vdma_open,
  .unlocked_ioctl = xilinx_vdma_unlocked_ioctl
};


static const struct file_operations xilinx_vdma_filter_fops = {
  .owner = THIS_MODULE,
  //	.llseek = no_llseek,
  .read  = xilinx_vdma_read_filter,
  .write = xilinx_vdma_write_filter,
  .open  = xilinx_vdma_open_filter,
				  ...
				  struct device_node *node)
{
  ...
  // Get the physical address of VDMA
  if(flag == 0){
    dev_info(chan->dev, "my_xvdma_original"); 
    chan->miscdev.minor = 0x44;
    chan->miscdev.name = "my_xvdma_original";
    chan->miscdev.fops = &xilinx_vdma_original_fops;
    chan->miscdev.parent = chan->dev; /* (1) */
    flag = 1;
  }
  else{
    dev_info(chan->dev, "my_xvdma_filter"); 
    chan->miscdev.minor = 0x43;
    chan->miscdev.name = "my_xvdma_filter";
    chan->miscdev.fops = &xilinx_vdma_filter_fops;
    chan->miscdev.parent = chan->dev; /* (1) */
  }
  ...
  err = misc_register(&chan->miscdev);
