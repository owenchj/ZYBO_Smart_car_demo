...
int vdma_init( Xuint32 uBaseAddr, Xuint32 ResolutionId, Xuint32 uMemAddr )
{
   ...
   //vdma_tx_start( uBaseAddr, ResolutionId, uMemAddr );

   return 0;
}

int vdma_init_sobel( Xuint32 uBaseAddr, Xuint32 ResolutionId, Xuint32 uMemAddr )
{
   vdma_rx_stop( uBaseAddr );
   vdma_rx_start_sobel( uBaseAddr, ResolutionId, uMemAddr );
   //  vdma_tx_start( uBaseAddr, ResolutionId, uMemAddr );
...
void vdma_change_addr( Xuint32 uBaseAddr, Xuint32 ResolutionId, Xuint32 uMemAddr )
{
   Xuint32 frame_width;
   ...
   Xuint32 frameAddress[3];

   frame_height = vres_get_height( ResolutionId );      // in lines
   frame_width  = vres_get_width ( ResolutionId ) * 3; // in bytes

   frame_stride = frame_width;   // choose by us
   frame_size   = frame_stride * frame_height;   // frame_size = frame_stride * height = 0xA00 * 480

   frameAddress[0] = uMemAddr + (frame_size * 0);
   frameAddress[1] = uMemAddr + (frame_size * 1);
   frameAddress[2] = uMemAddr + (frame_size * 2);

   *((volatile int *)(uBaseAddr+XAXIVDMA_S2MM_ADDR_OFFSET+XAXIVDMA_START_ADDR_OFFSET+0)) = frameAddress[1]; //0x30000000;  // DDR starting address of frame buffer-0
   *((volatile int *)(uBaseAddr+XAXIVDMA_S2MM_ADDR_OFFSET+XAXIVDMA_START_ADDR_OFFSET+4)) = frameAddress[2]; //0x30870000;  // DDR starting address of frame buffer-1
   *((volatile int *)(uBaseAddr+XAXIVDMA_S2MM_ADDR_OFFSET+XAXIVDMA_START_ADDR_OFFSET+8)) = frameAddress[3]; //0x310E0000;  // DDR starting address of frame buffer-2
   *((volatile int *)(uBaseAddr+XAXIVDMA_S2MM_ADDR_OFFSET+XAXIVDMA_STRD_FRMDLY_OFFSET )) = frame_stride;  // Frame delay is set to zero + Stride is set to 0x2000
...
int vdma_rx_start( Xuint32 uBaseAddr, Xuint32 ResolutionId, Xuint32 uMemAddr )
{
   ...
   *((volatile int *)(uBaseAddr+XAXIVDMA_RX_OFFSET+XAXIVDMA_CR_OFFSET                 )) = 0x00011043;  // S2MM_DMACR[0] : RS = 1 (Run) 0x00010043
   ...
   *((volatile int *)(uBaseAddr+XAXIVDMA_S2MM_ADDR_OFFSET+XAXIVDMA_VSIZE_OFFSET	      )) = frame_height;  // 1080 lines => 0x00000438
   ...
   timeout = VDMA_WAIT_TIMEOUT;
    while(--timeout)
    {
       ...
       x =   *((volatile int *)(uBaseAddr+XAXIVDMA_S2MM_ADDR_OFFSET+XAXIVDMA_SR_OFFSET  ));
...
int vdma_rx_start_sobel( Xuint32 uBaseAddr, Xuint32 ResolutionId, Xuint32 uMemAddr )
{
   int timeout;
   ...
   frame_height = vres_get_height( ResolutionId );      // in lines
   frame_width  = vres_get_width ( ResolutionId ); // in bytes
...
int vdma_tx_start( Xuint32 uBaseAddr, Xuint32 ResolutionId, Xuint32 uMemAddr )
{
   ...
   frame_stride = frame_width;   // choose by us
   frame_size   = frame_stride * frame_height;   // frame_size = frame_stride * height = 0x2000 * 1080
   ...
   *((volatile int *)(uBaseAddr+XAXIVDMA_TX_OFFSET+XAXIVDMA_CR_OFFSET                 )) = 0x0001008B;  // MM2S_DMACR[    0] : RS = 1 (Run) 0B
   ...
   *((volatile int *)(uBaseAddr+XAXIVDMA_MM2S_ADDR_OFFSET+XAXIVDMA_STRD_FRMDLY_OFFSET )) = frame_stride;  // Frame delay is set to zero + Stride is set to 0x2000
